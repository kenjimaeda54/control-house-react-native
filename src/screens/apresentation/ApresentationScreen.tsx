...
const data: IDataApresentation[] = [
  {
    ...
    description:
      'Com esta aplicação ira controlar seu dispositivo IOT',
  ...
  },
  {
    ...
    description:
      'Selecione um dos modos disponíveis para controlar   dispositivo',
  ...
  },
  {
    ...
    title: 'Cuidado!',
    description:
      'Existem modos que  precisam de configurações extras, não se preocupe, assim que selecionar será aberto um modal com instrução próxima etapa',
...
export default function ApresentationScreen() {
  const { currentBg, handleRow, currentIndex, navigateToHome } =
    useApresentaionViewModel()
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.primary }}>
      ...
      <View style={styles.viewDot}>
        {[...Array(data.length)].map((value, index) => (
          <View
            key={data[value]?.key}
            style={[
              styles.dot,
